...
The primary differences in the programming language Quil 1.0 (as appearing in pyQuil 1.3) and Quil 1.1 (as appearing in
2.0) amount to an enhanced memory model. Whereas the classical memory model in Quil 1.0 amounted to an flat bit array of
indefinite size, the memory model in  Quil 1.1 is segmented into typed, sized, named regions.
...
compiled program to the QPU for execution (and hence generate the expectation values). With Quil 1.1, this exact program
...
Quil 1.0 is not compatible with Quil 1.1 in the following ways:

-  The unnamed memory references ``[n]`` and ``[n-m]`` have no direct equivalent in Quil 1.1 and must be replaced by named memory references. (This primarily affects ``MEASURE`` instructions.)

-  The classical memory manipulation instructions have been modified: the operands of ``AND`` have been reversed (so that in Quil 1.1, the left operand is the target address) and ``OR`` has been replaced by ``IOR`` and its operands reversed (so that, again, in Quil 1.1 the left operand is the target address).

In all other instances, Quil 1.0 will operate identically with Quil 1.1.
...
::
    ...
    for bond_length in len(min_energies):
...
than writing to technical support with a job ID, users are expected to provide all pertinent details to how they
produced an error.
...
.. note::
    ...
    The QVM uses a legacy HTTP interface which will be replaced by a ``rpcq`` interface in a future release.
