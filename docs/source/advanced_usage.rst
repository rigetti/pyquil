...
A more sophisticated feature of pyQuil is that it can create templates of Quil programs.
An example use of these templates is in exponentiating a Hamiltonian that is parametrized by a constant.
This commonly occurs in variational algorithms. The function ``exponential_map`` is used to compute
exp[i * alpha * H] without explicitly filling in a value for alpha.
...
This ``parametric_prog`` function now acts as a template, caching the result of the ``exponential_map``
