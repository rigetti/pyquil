...
-   :pyPauliTerm now remembers the order of its operations.
    `sX(1)*sZ(2)` will compile to different Quil code than
    `sZ(2)*sX(1)`, although the terms will still be equal according to
    the `__eq__` method. During :pyPauliSum combination of like terms, a
    warning will be emitted if two terms are combined that have
    different orders of operation.
-   :pyPauliTerm.id() takes an optional argument `sort_ops` which
    ...
    previously. Use :pyPauliTerm.operations\_as\_set() instead. In the
    future, `sort_ops` will default to False and will eventually be
    removed.
-   :pyProgram.alloc() has been deprecated. Please instantiate
    :pyQubitPlaceholder() directly or request a "register" (list) of `n`
    placeholders by using the class constructor
    :pyQubitPlaceholder.register(n).
-   Programs must contain either (1) all instantiated qubits with
    integer indexes or (2) all placeholder qubits of type
    :pyQubitPlaceholder. We have found that most users use
...
-   Programs that contain qubit placeholders must be **explicitly
    ...
    :pyaddress\_qubits which returns a new program with all qubits
    indexed depending on the `qubit_mapping` argument. The original
    program is unaffected and can be "readdressed" multiple times.
-   :pyPauliTerm can now accept :pyQubitPlaceholder in addition to
    integers.
-   :pyQubitPlaceholder is no longer a subclass of :pyQubit.
    :pyLabelPlaceholder is no longer a subclass of :pyLabel.
-   :pyQuilAtom subclasses' hash functions have changed.
...
:pyCompilerConnection that will return sequences of pyquil programs,
...
In addition, the :pyCompilerConnection has another new method,
:pyapply\_clifford\_to\_pauli, which conjugates :pyPauliTerms by
:pyPrograms that are composed of Clifford gates. That is to say, given a
...
defaults for :pyProgram.measure\_all, and sensible defaults for
`classical_addresses` in :pyrun methods.

-   :pyProgram can be initiated with a generator expression.
-   :pyProgram.measure\_all (with no arguments) will measure all qubits
    in a program.
-   `classical_addresses` is now optional in QVM and QPU :pyrun methods.
    By default, any classical addresses targeted by `MEASURE` will be
    returned.
-   :pyQVMConnection.pauli\_expectation accepts `PauliSum` as arguments.
    This offers a more sensible API compared to
    :pyQVMConnection.expectation.
-   pyQuil will now retry jobs every 10 seconds if the QPU is re-tuning.
-   :pyCompilerConnection.compile now takes an optional argument `isa`
...
-   `shift_quantum_gates` has been removed. Users who relied on this
    functionality should use :pyQubitPlaceholder and :pyaddress\_qubits
