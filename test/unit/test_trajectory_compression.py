...
(:class:`pyquil.simulation._circuit.MergePlan`) which merges adjacent operations into
multi-qudit groups when ``max_subsystem_size >= 2``.  Compression must be a no-op on the *physics*: the
...
from pyquil.gates import CNOT, CZ, RX, RZ, H, X
from pyquil.noise._channels import Channel, MeasurementChannel, SuperopChannel
...
def _oracle_density(program, qubits, noise_model=None):
    ...
    sim = DensityMatrixSimulator(program, qubits=qubits, noise_model=noise_model, max_subsystem_size=0)
...
def _assert_real_compression(program, qubits, noise_model=None):
    ...
    assert len(compressed) < len(resolved), (
        f"compression did not merge any ops " f"({len(resolved)} resolved -> {len(compressed)} compressed)"
    )


def _trajectory_density(
    program, qubits, dims, n_traj, *, noise_model=None, seed=0, max_subsystem_size=MAX_SUBSYSTEM_SIZE
):
...
def _sampler_total_channel(program, qubits, *, noise_model=None, max_subsystem_size):
    ...
    operations = sim.operations(sim.linearize({}))
    dims = tuple(sim.dims)
    dimension = int(np.prod(dims))

    channel = np.eye(dimension * dimension, dtype=complex)
    for op, subsystem in operations:
        # Promote exactly as the kernel does, so a qubit-dimension channel on a register a
        # leakage model promoted to qutrits takes the same path here as in the sampler.
        matrix, _divisor, _is_measure = _op_to_kraus_matrix(_promote_to_register(op, tuple(subsystem), dims))
        kraus = qx.KrausMap.from_matrix(np.asarray(matrix), (tuple(dims[i] for i in subsystem),) * 2)
        embedded = qx.embed(qx.to_superop(kraus), target_dims=dims, positions=tuple(subsystem))
...
def _qubit_measurement_circuit_2():
    """The 2-qubit circuit with a terminal measurement and no noise.

    Every group is then all-unitary except the atomic instrument, which is the configuration
    the rank-one unitary shortcut in ``_op_to_kraus_matrix`` takes.
    """
    p, qubits, dims = _qubit_circuit_2()
    p += Measurement(qubit=Qubit(qubits[0]), classical_reg=None)
    return p, qubits, dims
...
def _qubit_circuit_5():
    ...
    p += CZ(7, 2)
    p += CZ(9, 4)
    for q, a in zip(QUBITS_5, (0.3, 0.7, 1.1, 0.5, 0.9), strict=False):
        p += RX(a, q)
    p += CNOT(2, 9)  # non-sorted relative to qubit ordering
    p += CNOT(4, 0)
    for q, a in zip(QUBITS_5, (0.2, 0.6, 0.4, 0.8, 1.0), strict=False):
        p += RZ(a, q)
...
def _depolarizing_model(insts, fidelity):
    ...
    return NoiseModel.from_channels([Channel.from_gate_fidelity(inst=inst, fidelity=fidelity) for inst in insts])
...
def test_compression_two_qubit_gates_no_noise():
    ...
    rho_uncompressed, _ = _trajectory_density(program, qubits, dims, n_traj=8, max_subsystem_size=0)
...
def test_compression_two_qubit_gates_with_noise():
    ...
    program, qubits, dims = _qubit_circuit_2()
    noise_model = _depolarizing_model([CNOT(2, 5), X(5), H(2), H(5)], fidelity=0.97)
    _assert_real_compression(program, qubits, noise_model)

    oracle = _oracle_density(program, qubits, noise_model)
    rho_est, _ = _trajectory_density(program, qubits, dims, n_traj=8000, noise_model=noise_model, seed=1)
...
def test_compression_five_qubit_gates_with_noise():
    ...
    program, qubits, dims = _qubit_circuit_5()
    noise_model = _depolarizing_model([CZ(7, 2), CZ(9, 4), CNOT(2, 9), CNOT(4, 0), CNOT(0, 7)], fidelity=0.99)
    _assert_real_compression(program, qubits, noise_model)

    oracle = _oracle_density(program, qubits, noise_model)
    rho_est, _ = _trajectory_density(program, qubits, dims, n_traj=12000, noise_model=noise_model, seed=2)
...
def test_compression_two_qutrit_gates_with_noise():
    ...
    oracle = _oracle_density(program, qubits, noise_model)
    rho_est, _ = _trajectory_density(program, qubits, dims, n_traj=8000, noise_model=noise_model, seed=3)
    assert float(qx.fidelity(rho_est, oracle)) > 0.99
...
def test_compression_five_qutrit_gates_with_noise():
    ...
    """
    program, qubits, dims = _qutrit_circuit_5()
    noise_model = _depolarizing_model([Gate("TSWAP", [], [2, 7]), Gate("TSWAP", [], [9, 4])], fidelity=0.99)
    _assert_real_compression(program, qubits, noise_model)

    oracle = _oracle_density(program, qubits, noise_model)
    rho_est, _ = _trajectory_density(program, qubits, dims, n_traj=8000, noise_model=noise_model, seed=4)
...
def test_compression_two_qubit_measurements_with_noise():
    ...
    oracle = _oracle_density(program, qubits, noise_model)
    rho_est, outcomes = _trajectory_density(program, qubits, dims, n_traj=8000, noise_model=noise_model, seed=7)
    assert float(qx.fidelity(rho_est, oracle)) > 0.99
...
def test_compression_five_qubit_measurements_with_noise():
    ...
    oracle = _oracle_density(program, qubits, noise_model)
    rho_est, outcomes = _trajectory_density(program, qubits, dims, n_traj=12000, noise_model=noise_model, seed=8)
    assert float(qx.fidelity(rho_est, oracle)) > 0.97
...
def test_measurement_outcome_column_order_under_compression():
    ...
    *i*-th ``MEASURE`` instruction.  This pins down that invariant directly by
    ...
    _, outcomes_compressed = _trajectory_density(program, qubits, dims, n_traj=16, seed=0, max_subsystem_size=2)
    _, outcomes_uncompressed = _trajectory_density(program, qubits, dims, n_traj=16, seed=0, max_subsystem_size=0)
...
def _noisy_channel_cases():
    ...
    """
    # Noiseless with measurements: all-unitary groups plus an atomic instrument, which is
    # the path the rank-one unitary shortcut in ``_op_to_kraus_matrix`` takes.
    pm, qm, _ = _qubit_measurement_circuit_2()
    yield ("noiseless_measure", pm, qm, None)
    ...
    yield (
        ...
        p5,
        q5,
        _depolarizing_model([CZ(7, 2), CZ(9, 4), CNOT(2, 9), CNOT(4, 0), CNOT(0, 7)], fidelity=0.99),
    )
...
def test_compression_preserves_total_channel_exactly(case):
    ...
    reference = _sampler_total_channel(program, qubits, noise_model=noise_model, max_subsystem_size=0)
    for size in (1, 2):
        channel = _sampler_total_channel(program, qubits, noise_model=noise_model, max_subsystem_size=size)
        max_abs_diff = float(np.abs(channel - reference).max())
        assert max_abs_diff < 1e-10, (
            f"compression at max_subsystem_size={size} changed the channel " f"(max |ΔS| = {max_abs_diff:.2e})"
        )
...
# ══════════════════════════════════════════════════════════
# Tightened: merging must not discard low-weight error branches
# ══════════════════════════════════════════════════════════


def test_merging_preserves_correlated_multi_error_branches():
    """A merged group keeps the ``p^2`` branch its constituents produce together.

    Two rank-2 Pauli channels compose to a rank-4 one whose fourth Choi eigenvalue is the
    product of the two error rates.  A Kraus conversion that truncated at a fixed ``1e-6``
    dropped it outright, which made ``max_subsystem_size`` change the simulated channel and
    made the resulting map non-trace-preserving.  Those branches are exactly the correlated
    multi-error events a QEC logical-error-rate estimate is measuring, so a discrepancy far
    below any sampling tolerance still matters.
    """
    error_rate = 5e-4
    program = Program(Gate("X", [], [Qubit(0)]), Gate("Y", [], [Qubit(0)]))
    noise_model = NoiseModel.from_channels(
        [
            SuperopChannel.from_pauli_noise(inst=Gate("X", [], [Qubit(0)]), pauli_noise={"X": error_rate}),
            SuperopChannel.from_pauli_noise(inst=Gate("Y", [], [Qubit(0)]), pauli_noise={"Z": error_rate}),
        ]
    )

    reference = _sampler_total_channel(program, [0], noise_model=noise_model, max_subsystem_size=0)
    merged = _sampler_total_channel(program, [0], noise_model=noise_model, max_subsystem_size=2)

    # The discarded branch had weight p^2 = 2.5e-7, orders of magnitude above this tolerance.
    assert float(np.abs(merged - reference).max()) < 1e-12

    # ... and the merged channel stays trace preserving, which truncation broke.
    for basis_state in range(2):
        rho = np.zeros((2, 2), dtype=complex)
        rho[basis_state, basis_state] = 1.0
        evolved = (merged @ rho.reshape(-1)).reshape(2, 2)
        assert abs(np.trace(evolved) - 1.0) < 1e-12
