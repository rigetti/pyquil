...
def _simulate_trajectories(program, noise_model=None, qubits=None, num_trajectories=1, batch_size=256, random_seed=0):
    """Build + compress + run trajectories, returning ``(psi, outcomes)``."""
    sim = TrajectorySimulator(program, noise_model=noise_model, qubits=qubits)
    operations = sim.adapt(sim.compress(sim.resolve()))
...
class TestMultiDeviceTrajectory:
    ...
    def test_sample_results_match_single_device(self):
        ...
        out_default = sim_default.sample(None, num_trajectories=64, batch_size=16, random_seed=99)
        out_explicit = sim_explicit.sample(None, num_trajectories=64, batch_size=16, random_seed=99)
    ...
    def test_noisy_sample_with_devices(self):
        ...
        outcomes = sim.sample(None, num_trajectories=1024, batch_size=256, random_seed=7)
