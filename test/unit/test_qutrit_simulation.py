...
from pyquil.noise._channels import Channel, MeasurementChannel, SuperopResetChannel
...
def _sample(program, qubits=None, noise_model=None, num_trajectories=1000, batch_size=250, random_seed=0):
    """Run trajectory sampling, returning outcomes."""
    sim = TrajectorySimulator(program, qubits=qubits, noise_model=noise_model)
    return sim.sample(None, num_trajectories=num_trajectories, key=jax.random.key(random_seed), batch_size=batch_size)
...
class TestQutritMeasurements:
    ...
    def test_qutrit_measure_ground_state(self):
        ...
        outcomes = _sample(p_ground, qubits=[0], num_trajectories=100, random_seed=42)
    ...
    def test_qutrit_measure_excited_state(self):
        ...
        outcomes = _sample(p, qubits=[0], num_trajectories=100, random_seed=42)
    ...
    def test_qutrit_measure_superposition_statistics(self):
        ...
        n_traj = 3000
        outcomes = _sample(p, qubits=[0], num_trajectories=n_traj, random_seed=123)
    ...
    def test_qutrit_noisy_measurement_channel(self):
        ...
        meas_ch = MeasurementChannel.from_readout_fidelity(inst=meas_inst, fidelity=0.9, dim=3)
    ...
    def test_mixed_qubit_qutrit_measurement(self):
        ...
        outcomes = _sample(p, qubits=[0, 1], num_trajectories=50, random_seed=0)
...
class TestQutritNoiseChannels:
    ...
    def test_qutrit_reset_channel(self):
        ...
        reset_ch = SuperopResetChannel.from_reset_fidelity(inst=reset_inst, fidelity=0.9, dim=3)
    ...
    def test_mixed_noise_qubit_and_qutrit(self):
        ...
        # Noisy qubit X gate
        ch_qubit = Channel.from_gate_fidelity(inst=Gate("X", [], [0]), fidelity=0.95)
        # Noisy qutrit TX gate
        ch_qutrit = Channel.from_gate_fidelity(inst=Gate("TX", [], [1]), fidelity=0.95)
...
class TestQutritTrajectoryBatching:
    ...
    def test_qutrit_trajectory_outcomes_shape(self):
        ...
        outcomes = _sample(p, qubits=[0], num_trajectories=500, batch_size=100, random_seed=0)
