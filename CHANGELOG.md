...
-   In addition to the simultaneous 1Q RB fidelities contained in device 
    ...
    individual gate operation. The names of these fields have been changed 
    for clarity, and standard errors for both fidelities have been added 
    as well. Finally, deprecation warnings have been added regarding the 
    ``fCPHASE`` and ``fBellState`` device spec fields, which are no longer
