...
from ._classical_computations import build_extern_function_signatures
...
class RandomizedCompilingConfiguration:
    ...
    * generate source unitaries for the gate program.
    ...
    def _validate(self) -> None:
        if self.base_cycle_repetitions > 1:
    ...
    def _managed_u2_layer_count(self) -> int:
        ...
        """
        return self.cycle_count + 1 - int(self.skip_first_layer) - int(self.skip_final_layer)
    ...
    def _get_unitary_memory_index(self, layer_index: int) -> int:
        ...
        if self.skip_final_layer and layer_index == self.cycle_count:
    ...
    def _seed_length(self) -> int:
        return math.ceil(self.cycle_count / self._paulis_per_value)
    ...
    def apply_pauli_pair(
        ...
        if layer_index == self.cycle_count:
            next_pauli = PauliLiteral.I
        else:
