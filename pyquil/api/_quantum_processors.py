...
def get_device(client: Client, quantum_processor_id: str) -> Device:
    # STUB FOR NOW ################
    with client._http_client() as client:
        res = client.get(f"https://forest-server.qcs.rigetti.com/devices/{quantum_processor_id}")
        if res.status_code >= 400:
            raise _parse_error(res)
    return Device(quantum_processor_id, res.json()["device"])
